...
With great power come responsibility.  Just because you can do that doesn't
mean you should.
...
However, maybe the data can be paired up with the types you specified in
`types`. For example:
...
```python >>> converted = [func(val) for func, val in zip(types, row)] >>>
